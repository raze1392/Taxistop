...
window.chanakya.Map.Directions = (function () {
  var getDirections = function (source, destination) {
    ...
    chanakya.Map.Details.directionsService.route(request, function(response, status) {
      if (status == google.maps.DirectionsStatus.OK) {
        chanakya.Map.clearMarkers();
        chanakya.Map.Details.directionsDisplay.setDirections(response);
      }
    });
  }
  ...
  var setTravelMode = function(travelMode) {
    ...
    if (chanakya.Map.Details.Directions.TravelMode[travelMode]) {
      chanakya.Map.Details.Directions.travelModeSelected = chanakya.Map.Details.Directions.TravelMode[travelMode]
    } else {
      chanakya.Map.Details.Directions.travelModeSelected = chanakya.Map.Details.Directions.TravelMode.driving;
    }
  ...
  var getTravelMode = function() {
    return chanakya.Map.Details.Directions.travelModeSelected;
  }

  var setRouteAlternatives = function(arg) {
    if (typeof arg === "boolean") chanakya.Map.Details.Directions.routeAlternatives = arg;
    else chanakya.Map.Details.Directions.routeAlternatives = false;
  }

  var getRouteAlternatives = function(arg) {
    return chanakya.Map.Details.Directions.routeAlternatives;
  }

  var setUnitSystem = function(unit) {
    unit = unit.toLowerCase();
    if (chanakya.Map.Details.Directions.UnitSystem[unit]) {
      chanakya.Map.Details.Directions.unitSystemSelected = chanakya.Map.Details.Directions.UnitSystem[unit]
    } else {
      chanakya.Map.Details.Directions.unitSystemSelected = chanakya.Map.Details.Directions.UnitSystem.metric;
    }
  ...
  var getUnitSystem = function() {
    return chanakya.Map.Details.Directions.unitSystemSelected;
  }
